...
def safe_type_name(name: str) -> str:
    """Make a class or enum identifier safe for Python-oriented generators."""
    normalized = safe_name(name)
    normalized = re.sub(r"[^0-9A-Za-z_]", "_", normalized)
    normalized = re.sub(r"_+", "_", normalized)
    normalized = normalized.strip("_") or "_"
...
SOURCE_TTLS = [
    SCRIPT_DIR / "d3fend.ttl",
    SCRIPT_DIR / "d3fend-protege.ttl",
]
...
MAPPINGS_JSON_PATH = SCRIPT_DIR.parent / "mappings" / "d3fend-full-mappings-1.3.0.json"
...
for c in all_classes:
    class_uri_to_name[str(c)] = safe_type_name(local_name(c))
...
def range_name(uri):
    ...
    if s.startswith(str(D3F)):
        return safe_type_name(local_name(uri))
    return None
...
for type_name in sorted(enum_classes):
    ...
    enum_name = f"{safe_type_name(type_name)}Enum"
    ...
    for ind_name in inds:
        safe_ind = safe_type_name(ind_name)
...
for p in all_obj_props:
    ...
    # domain hint
    if domains:
        d3f_domains = [safe_type_name(local_name(d)) for d in domains if d.startswith(str(D3F))]
        if d3f_domains:
            emit(f"    domain_of:")
    ...
    if d3f_ranges:
        rn = safe_type_name(local_name(d3f_ranges[0]))
        emit(f"    range: {rn}")
    else:
        emit(f"    range: string")
...
for p in all_obj_props + all_data_props:
    ...
    for d in g.objects(p, RDFS.domain):
        if isinstance(d, URIRef) and str(d).startswith(str(D3F)):
            class_domain_slots[safe_type_name(local_name(d))].append(p_name)
...
for c in all_classes:
    ...
    if len(parents_uris) == 1:
        parent_name = safe_type_name(local_name(parents_uris[0]))
        emit(f"    is_a: {parent_name}")
    elif len(parents_uris) > 1:
        parent_name = safe_type_name(local_name(parents_uris[0]))
        emit(f"    is_a: {parent_name}")
        mixins = [safe_type_name(local_name(p)) for p in parents_uris[1:]]
        emit(f"    mixins:")
        for m in mixins:
            emit(f"      - {m}")
    ...
    if alt_labels:
        ...
        emit(f"    aliases:")
        emit(f"      - {yaml_str_inline(c_local)}")
...
output_path = SCRIPT_DIR / "d3fend.yaml"
